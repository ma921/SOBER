...
class _MaxSAT(TestFunction):
	def __init__(self, data_path, random_seed, normalize=False,  **kwargs):
...
class MaxSAT28(_MaxSAT):
	def __init__(self, data_path, random_seed=None):
		super().__init__(data_path, random_seed=random_seed)
    
def setup_maxsat():
    """
    Set up the experiments with MaxSat task
    
    Return:
    - prior: class, the function of binary prior
    - TestFunction: class, the function that returns true function value
    """
    MAXSAT_DIR_NAME = '../experiments/dataset/'
    data_filename = 'maxcut-johnson8-2-4.clq.wcnf'
    data_path = os.path.join(MAXSAT_DIR_NAME, data_filename)
    maxsat_ = MaxSAT28(data_path)
